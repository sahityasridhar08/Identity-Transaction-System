...
<?php
	...
	$sql="insert into cafe(id,ino,name,price) values($id,$iid,'$bname',$amt)";
	if($valid==true){
	$result=mysqli_query($link,$sql);
		if($result==true){
			...
			$iid=0;
			$iname="";
			$sql="update user_personal set balance=balance-$amt where id=$id";
			$result=mysqli_query($link,$sql);
			$amt=0;
			$id=0;
			//$error="";
...
<form class="formoid-solid-orange" style="background-color:#ffffff;font-size:14px;font-family:'Roboto',Arial,Helvetica,sans-serif;color:#34495E;max-width:480px;min-width:150px" method="post"><div class="title"><h2>Cafe Entry</h2></div>
<?php echo $error ?>	
...
<div class="element-number"><label class="title">Enter Item ID</label><div class="item-cont"><input id="no_chldren" class="large" type="number" min="0" max="100" name="bid" placeholder="Item ID" value="<?php echo $iid?>" /><span class="icon-place"></span></div></div>

<div class="element-input"><label class="title">Item Name</label><div class="item-cont"><input class="large" type="text" name="bname" placeholder="Item Name" value="<?php echo $iname?>" /><span class="icon-place"></span>
<div class="element-number"><label class="title">Enter Amount Paid</label><div class="item-cont"><input id="no_chldren" class="large" type="number" min="0" max="100" name="fine" placeholder="Amount" value="<?php echo $amt?>" /><span class="icon-place"></span></div></div>
